...
class CoachTrainer(object):
    ...
    def train_step(self, X, lengths, y_bin, y_final):
        ...
        pred_entityname_list, gold_entityname_list = self.entityname_predictor(
            lstm_hiddens, binary_golds=y_bin, final_golds=y_final
        )
        for pred_entityname_each_sample, gold_entityname_each_sample in zip(
            pred_entityname_list, gold_entityname_list
        ):
            if pred_entityname_each_sample is not None:
                assert (
                    pred_entityname_each_sample.size()[0]
                    == gold_entityname_each_sample.size()[0]
                )
                loss_entityname = self.loss_fn(
                    pred_entityname_each_sample, gold_entityname_each_sample.cuda()
                )
                self.optimizer.zero_grad()
    ...
    def evaluate(self, dataloader, tgt_dm, use_bilstm):
        ...
        for i, (X, lengths, y_bin, y_final) in pbar:
            ...
            bin_preds_batch, lstm_hiddens = self.binary_ner_tagger(
                X, return_hiddens=True
            )
            bin_preds_batch = self.binary_ner_tagger.crf_decode(
                bin_preds_batch, lengths
            )
            binary_preds.extend(bin_preds_batch)

            entityname_preds_batch = self.entityname_predictor(
                lstm_hiddens,
                binary_preditions=bin_preds_batch,
                binary_golds=None,
                final_golds=None,
            )

            final_preds_batch = self.combine_binary_and_entityname_preds(
                bin_preds_batch, entityname_preds_batch, tgt_dm
            )
        ...
        for bin_pred, bin_gold, final_pred, final_gold in zip(
            binary_preds, binary_golds, final_preds, final_golds
        ):
    ...
    def combine_binary_and_entityname_preds(
        self, binary_preds_batch, entityname_preds_batch, tgt_dm
    ):
        """
        Input:
