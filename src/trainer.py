...
class BertSpanTypeTrainer(object):
    def __init__(self, params, span_model, type_model):
        ...
        self.optimizer = torch.optim.Adam(
            [self.span_model.parameters()] + [self.type_model.parameters()],
            lr=params.lr,
        )
    ...
    def train_step(self, X, labels_bio, y):
        """
        need to return: loss, loss_span and loss_type
        """
        self.span_model.train()
        self.type_model.train()
        output_span = self.span_model(X, labels_bio)
        output_type = self.type_model(X, y, labels_bio)

        loss_span = self.loss_fn(output_span, labels_bio)
        loss_type = self.loss_fn(output_type, y)
